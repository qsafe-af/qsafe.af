...
import {
  ...
  decodeAddress as polkadotDecodeAddress,
  cryptoWaitReady,
} from "@polkadot/util-crypto";

// Cache for chain properties
const chainPropertiesCache = new Map<
  string,
  {
    ss58Format: number;
    tokenSymbol?: string;
 ...
 */
export function encodeAddressSync(
  address: string | Uint8Array,
  ss58Format = 42,
): string {
  if (!cryptoReady) {
 ...
 */
export function decodeAddressSync(
  address: string,
  ignoreChecksum?: boolean,
  ss58Format?: number,
): Uint8Array {
 ...
 */
export async function initializeSS58(
  endpoint?: string,
  genesis?: string,
): Promise<void> {
  await ensureCryptoReady();
