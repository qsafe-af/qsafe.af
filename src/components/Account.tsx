...
const Account: React.FC = () => {
  const { chainId, accountId } = useParams<{ chainId: string; accountId: string }>();
    ...
    try {
      ...
      if (accountId.startsWith('0x')) {
        ...
        if (chain) {
          ...
          if (ss58Format === undefined && chain.endpoints && chain.endpoints.length > 0) {
            try {
              const properties = await fetchSystemProperties(chain.endpoints[0], chain.genesis);
      ...
      if (filter === 'transfers') {
        ...
        console.log('BalanceEventsConnection:', eventsData.balanceEventsConnection);
        console.log('Total Count:', eventsData.balanceEventsConnection?.totalCount);
    ...
    return (
      <div className="container mt-5">
        <Alert variant="warning">
          ...
          <p>Account history is not available for this chain as no indexer is configured.</p>
    ...
    <div className="container mt-4">
      ...
      {error && (
        <Alert variant="danger" className="mb-4" dismissible onClose={() => setError(null)}>
          ...
          <Button variant="outline-danger" size="sm" onClick={() => { setCursor(null); setEndCursor(null); loadAccountData(); }}>
            Retry
          </Button>
        </Alert>
      )}
                ...
                <div className={`${themeClasses.bg.subtle} p-3 rounded`}>
                  {accountData.free && (
                    <div className="mb-2">
                      ...
                      <div className="font-monospace">{formatAmount(accountData.free)}</div>
                    </div>
                  )}
                  {accountData.reserved && (
                    <div className="mb-2">
                      <small className="text-muted">Reserved:</small>
                      <div className="font-monospace">{formatAmount(accountData.reserved)}</div>
                    </div>
                  )}
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <div className="font-monospace fw-bold">
                          {formatAmount(
                            (BigInt(accountData.free || '0') + BigInt(accountData.reserved || '0')).toString()
                          )}
                        </div>
                      </div>
                    </>
      ...
      <Card className={`${themeClasses.card} mb-4`}>
        <Card.Header>
          <div className="d-flex justify-content-between align-items-center">
            ...
            <Nav variant="pills" activeKey={filter} onSelect={(k) => handleFilterChange(k as EventFilter)}>
              <Nav.Item>
        ...
        <Card.Body>
          {loading && events.length === 0 ? (
            ...
            <div className="text-center py-5 text-muted">
              <p className="mb-0">No {filter !== 'all' ? filter : 'transactions'} found for this account</p>
            </div>
          ) : (
            <>
              <Table responsive hover className={themeClasses.table}>
                ...
                <tbody>
                  {filteredEvents.map((event) => (
                    <tr key={event.id}>
                      <td>
                        <Link to={`/chains/${chainId}/block/${event.blockHeight}`} className="text-decoration-none">
                          <Badge bg="secondary">{event.blockHeight.toLocaleString()}</Badge>
                        </Link>
                      </td>
                      <td>
                        <small>
                      ...
                      <td>
                          ...
                          <div className="small">
                            {event.from === ss58Address ? (
                              <>
                                ...
                                <Link to={`/chains/${chainId}/account/${event.to}`} className="font-monospace text-decoration-none">
                                  {event.to}
                                </Link>
                              </>
                              ...
                              <>
                                ...
                                <Link to={`/chains/${chainId}/account/${event.from}`} className="font-monospace text-decoration-none">
                                  {event.from}
                                </Link>
                              </>
                            ) : (
                        ...
                        {(event.type.includes('Burned') || event.type.includes('Slashed')) && event.accountId && (
