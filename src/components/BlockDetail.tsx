...
import {
  parseExtrinsicHeaderAndCall,
  toHuman,
} from "../utils/polkadot/extrinsicDecoder";
import {
  systemEventsStorageKey,
  decodeEventsAtBlock,
} from "../utils/polkadot/eventDecoder";
...
const BlockDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ws.onopen = async () => {
      ...
      try {
        ...
        // Fetch runtime version for this block
        const runtimeVersion = (await sendRequest("state_getRuntimeVersion", [
          blockHash,
        ])) as { specName: string; specVersion: number };
        // Fetch metadata with caching based on spec version
        const metadataInfo = await fetchMetadata(endpoint, blockHash);
        const {
          callMap,
          ss58Format: ss58FromMeta,
          registry,
          metadata,
        } = metadataInfo;
        ...
        const eventsHex: string | null = (await sendRequest(
          "state_getStorageAt",
          [eventsKey, blockHash],
        )) as string;
        ...
        for (let idx = 0; idx < extrinsics.length; idx++) {
          ...
          try {
            const info = (await sendRequest("payment_queryInfo", [
              hex,
              blockHash,
            ])) as {
        ...
        const digestInfo = block.block.header.digest
          ? decodeDigest(block.block.header.digest)
          : null;
  ...
  const transactions =
    blockData.extrinsicsData?.filter(
      (ext) =>
        ext.parsed.isSigned &&
        ext.events?.transfers &&
        ext.events.transfers.length > 0,
    ) || [];
  ...
  return (
    <div className="container mt-4 block-detail-container">
      ...
      <Card
        className={`${themeClasses.card} block-info-card block-detail-section`}
      >
        <Card.Header>
        ...
        <Card.Body>
          ...
          {blockData.runtimeSpecVersion !== undefined && (
            <div className="row mt-3">
              <div className="col-12">
                <h6 className="block-section-header">Runtime</h6>
                <p className="font-monospace small text-break">
                  <Link
                    to={`/chains/${chainId}/runtime/v${blockData.runtimeSpecVersion}`}
                    className="hash-link"
                  >
                    {blockData.runtimeSpecName
                      ? `${blockData.runtimeSpecName} v${blockData.runtimeSpecVersion}`
                      : `v${blockData.runtimeSpecVersion}`}
                  </Link>
                </p>
              </div>
            </div>
        ...
        <Card className={`${themeClasses.card} block-detail-section`}>
          ...
          <Card.Body>
            <Table
              responsive
              hover
              className={`${themeClasses.table} transactions-table`}
            >
              <thead>
                <tr>
                  <th>Extrinsic</th>
                      ...
                      <tr className="tx-data-row">
                        <td>
                          ...
                          >
                            <Badge
                              bg="secondary"
                              className="extrinsic-index-badge"
                            >
      ...
      {blockData.extrinsicsData && blockData.extrinsicsData.length > 0 && (
        <Card className={`${themeClasses.card} block-detail-section`}>
          <Card.Header>
            <h5 className="mb-0">
              Extrinsics ({blockData.extrinsicsData.length})
            </h5>
          </Card.Header>
          <Card.Body>
                  ...
                  <Card.Body>
                    {ext.parsed.isSigned && (
                      <div className="extrinsic-details mb-3">
                        <div className="extrinsic-detail-row">
                          <span className="extrinsic-detail-label">
                            Signer:
                          </span>
                          {ext.parsed.sender ? (
                            <Link
                          ...
                          <div className="col-md-6">
                            {ext.parsed.tipHuman && (
                              <div className="extrinsic-detail-row">
                                ...
                                <span className="font-monospace small amount-display">
                                  {ext.parsed.tipHuman}
                                </span>
                              </div>
                            )}
                            ...
                            <tbody>
                              {ext.events.transfers.map((transfer, i) => (
                                <tr key={i}>
                                  ...
                                  <td className="border-0 text-center arrow-cell">
                                    →
                                  </td>
                    ...
                    {ext.events?.feePaid && (
                      <div className="extrinsic-detail-row mt-2">
                        ...
                        <div>
                          <span className="font-monospace small amount-display">
                            {ext.events.feePaid.amountHuman}
                          </span>{" "}
                          by{" "}
                          <Link
